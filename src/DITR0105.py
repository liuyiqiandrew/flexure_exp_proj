...
class pll_reader:
    def __init__(self, ctl, save_duration=10, save_path='caliper_output.txt') -> None:
        self.phase_error = 0
        self.centering_error = 0
        self.delays = np.zeros(2)
        self.ctl = ctl
        self.start = None
    ...
    def __del__(self):
        self.fstream.close()
        print("out stream closed")

    def pll_read(self):
        # set start time
        if self.start is None:
            self.start = time.time()
        # if time elapse is less than requested, continue recording.
        time_elapsed = time.time() - self.start
        if time_elapsed < self.save_duration:
            threading.Timer(PACKET_SEPARATION - self.phase_error - self.centering_error, self.pll_read).start()
        
        data = self.ctl.read(SPI_WINDOW*SPI_WINDOW_N)
        record_time = datetime.now()
        clk, data = parse_data(data)
        if self.pll_adjust(clk):
            
            values = decode_spi(clk, data)
            save_string = f'{record_time} | {values:>6.03f} | {self.cur_delay_error:>8.05f}\n'
            self.fstream.write(save_string)
            print(f'Saved: {save_string.strip()}')
            
            # print(values)
        
    def pll_adjust(self, clk):
        delay_error = adjust_delay(clk)
        self.cur_delay_error = delay_error
        # print(delay_error)
